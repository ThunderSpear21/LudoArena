    ...
    try {
      ...
      if (data.type === "get_players") {
        ...
        if (room) {
          ...
          //ws.send(JSON.stringify({ type: "player_list", players, hostId }));

          wss.clients.forEach((client) => {
            if (client.readyState === WebSocket.OPEN) {
              client.send(
                JSON.stringify({ type: "player_list", players, hostId })
              );
            }
          });
        }
      ...
      // New event to handle piece movement
        ...
        if (!room.gameState.positions[data.playerId]) {
          room.gameState.positions[data.playerId] = [
            ...defaultPositions[playerColor],
          ];
        }

        console.log(
          "📍 Current positions:",
          JSON.stringify(room.gameState.positions, null, 2)
        );
        ...
        const isInStartArea = defaultPositions[playerColor].some(
          (startPos) =>
            startPos[0] === piecePosition[0] && startPos[1] === piecePosition[1]
        );
        ...
        const rolledSix = room.gameState.diceRoll === 6;
        // Move to next player
        if (!rolledSix) {
          // Move to next player only if dice roll wasn't 6
        ...
        console.log(
          `🚀 Player ${data.playerId} moved piece ${pieceIndex} to ${newPosition}`
        );
        ...
        console.log(
          `🔗 Player ${data.username} joined room ${data.roomId} via WebSocket`
        );
        ...
        // Optionally: send confirmation to the joining client
