...
class WaitingLobbyScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => WaitingLobbyBloc(roomId: roomId)..add(LoadPlayers(roomId)),
...
class _WaitingLobbyScreenBody extends StatefulWidget {
  ...
  @override
  State<_WaitingLobbyScreenBody> createState() => _WaitingLobbyScreenBodyState();
}
            ...
            children: [
                  ...
                  if (state is LobbyLoading) {
                    ...
                    return Column(
                      ...
                      children: state.players
                          .map((player) => Text(player, style: const TextStyle(fontSize: 16)))
                          .toList(),
                    );
              ...
              Center(
                child: ElevatedButton(
                  onPressed: () {
                    final currentState = context.read<WaitingLobbyBloc>().state;
                    if (currentState is LobbyLoaded) {
                      context.read<WaitingLobbyBloc>().add(StartGame(widget.roomId, currentState.players));
                    }
                  },
